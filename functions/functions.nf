...
def validateParams() {
    ...
    // [PARAMS VALIDATION]
    // (note)! splitting $params at ", " rather than "," ensures that the string is splitted only 
    // at the comma that delimits the entries rather than commas within params values itself, 
    // such as in [fastq:*_{1,2}.fastq.gz, bam:my.bam]
    
    def existing_params = []
    "${params}".toString().replaceAll(", ", "\n").readLines().eachWithIndex { item, index ->

        def param_name = item.split(":")[0].replaceAll("\\[|\\s|\\]", "")
        existing_params << param_name
        
    }

    // list allowed parameters based on nextflow.config:
    def valid_params =[]
    new File("${baseDir}/nextflow.config").withReader { Reader reader ->

        reader.eachLine { String line ->

            def current = line.replaceAll("\\s", "")

            // this collects all entries between "params{" and "}", removing empty lines and comments
            ...
            // collect the params, also those of type "params.*" rather than params{...}
            if(go && !current.startsWith("/") && !current.startsWith("params{") && current != ''){
                valid_params = valid_params << "$current".split("=")[0].replaceAll("params.", "")
            }

        }
    }
    // make unique
    valid_params = valid_params.unique { a, b -> a <=> b }

    // invalid params are all params minus allowed params:
    diff = existing_params.minus(valid_params)

    // summary of invalid params, if existing throw error and exit 1:

    if(diff.size() > 0){

        println("$ANSI_RED")
        println("-----------------------------------------------------------------------------------------------")
        println("[Error] Invalid params not listed in nextflow.config:")
        diff.each { file -> println("=> $file") }
        println("-----------------------------------------------------------------------------------------------")
        println("$ANSI_RESET")
    ...
    // once params validation has passed validate version:
    checkNfVersion()

    // and print intro:
    introMessage()

    // and print params summary:
    println("$ANSI_YELLOW")
    println("-----------------------------------------------------------------------------------------------")
    println("[FULL PARAMETER SUMMARY]")
    println("")
    "$params".replaceAll(", ", "\n").readLines().eachWithIndex { item, index ->
        def print_that=item.replaceAll("\\[|\\]", "")
        println("=> $print_that")
    }
    println("-----------------------------------------------------------------------------------------------")
    println("$ANSI_RESET")
